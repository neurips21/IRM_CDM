...
from models import MLP, Discriminator
...
def run_colored_mnist_plus(
    ...
    """Run CMNIST+ experiment and return train/test accuracies."""

    if method in ["ERM", "MMD", "CGAN", "REx", "Oracle"]:
    ...
    if method in ["ERM", "IRM", "IRMBAL", "REx",  "Oracle"]:
        assert(cdm_penalty_weight==0)
    ...
    suffix = 'CMNIST_PLUS'+'_color_prob_' + str(color_prob) + '_imba_' + str(imba)
    ...
    n_classes = 2
    train_envs = {env_name: convert_examples_to_features(examples, device, method, training=True)
                  for env_name, examples in train_examples.items()}
    test_envs = {env_name: convert_examples_to_features(examples, device, method, training=False)
                 for env_name, examples in test_examples.items()}
    all_envs = [env_name for env_name in it.chain(train_envs, test_envs)]

    # save_image_local(train_envs['train0']['images'],train_envs['train0']['labels'],0)
    ...
    # weights for JSGAN
    weights = torch.FloatTensor([[1-imba,imba],[imba,1-imba]]).to(device)

    for restart in range(n_restarts):

        # Initialize model
        model = MLP(hidden_dim, n_classes, 
        grayscale_model=grayscale_model).to(device)

        if method in ["CGAN", "IRMCGAN"]:
            D = Discriminator(hidden_dim, n_layers=n_layers_D).to(device)
            ...
            if method in ["CGAN", "IRMCGAN"]:
                for substep in range(substeps):
                    for _, env in it.chain(train_envs.items(), test_envs.items()):
                        X_env = env['images']
                        X_env = X_env.view(X_env.shape[0], 3, 14, 14).float()

                        y_env = env['labels']
                        reps, logits = model(X_env)  # multi-class logit
                    ...
                    # cdm_penalty
                    cdm_penalty = calculate_cgan_loss(train_envs.items(), D, weights, cgan_type=cgan_type)
...
def main():
    ...
    parser.add_argument('--datadir', type=str, default='./colored_mnist/data_local/',
                        help='directory containing CMNIST+ datasets')
    
    parser.add_argument('--resdir', type=str, default='./colored_mnist/results/',
                        help='directory for results')
    
    parser.add_argument('--color_prob', type=float, default=0.9, help='P(C | Y,E) of the tr envs')
    parser.add_argument('--imba', type=float, default=0.9, help='class imba of tr envs')
    parser.add_argument('--method', type=str, default='Oracle')
    ...
    parser.add_argument('--n_layers_D', type=int, default=1)
    ...
    parser.add_argument('--penalty_weight', type=float, default=0.0)
    parser.add_argument('--cdm_penalty_weight', type=float, default=0.0)
    ...
    # parser.add_argument('--grayscale_model', type=bool, default=False)
    ...
    run_colored_mnist_plus(**vars(args))
